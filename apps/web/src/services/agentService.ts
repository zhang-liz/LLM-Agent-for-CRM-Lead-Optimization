import type { Lead, TeamMetrics, Recommendations, Interaction } from '@leadloop/shared';
...
export async function getSystemStatus(): Promise<SystemStatus | null> {
  try {
    ...
    if (!healthRes.ok || !configRes.ok) return null;
    const health = await healthRes.json();
    const config = await configRes.json();
    return {
      sentimentProvider: health.sentimentProvider ?? 'keyword',
      apiOk: Boolean(health.ok),
      attributionMode: config.attributionMode ?? 'time_decay',
      timeDecayLambda: Number(config.timeDecayLambda ?? 0.1)
    };
  } catch {
    return null;
  }
